...
try {
    ...
    const result =
        await $`echo "Yes, I want to truncate 2 tables" | bunx drizzle-kit push --force`.quiet();

    if (result.exitCode === 0) {
        console.log('✅ Database schema migrated successfully!');
        console.log(
            "⚠️  Note: User and session tables were truncated. You'll need to create new accounts.",
        );
    } else {
